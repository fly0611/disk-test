...
#
mkdir ./disk-all
./nmon -f -s 2 -c 1900 -m ./disk-all &
for i in `cat all_disk.data`
do
fio --ioengine=libaio --randrepeat=0 --norandommap --thread --direct=1 --group_reporting --name=all_disk --ramp_time=30 --runtime=3600 --time_based --numjobs=1 --iodepth=32 --filename="$i" --rw=read --bs=2048k --output=./disk-all/`basename ${i}`_2048K_seqW.logout &
done
